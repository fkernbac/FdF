...
/*   Updated: 2022/08/30 19:24:21 by fkernbac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void mlx_update_matrix(const mlx_t* mlx, int32_t width, int32_t height)
{
	 ...
	 * to keep updating.
	 */
	width = mlx_settings[MLX_STRETCH_IMAGE] ? mlxctx->initialWidth : (uint32_t)mlx->width;
	height = mlx_settings[MLX_STRETCH_IMAGE] ? mlxctx->initialHeight : (uint32_t)mlx->height;
